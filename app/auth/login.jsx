...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!snapshot.exists()) { Alert.alert("Accès refusé", "Votre profil n'a pas été trouvé."); setLoading(false); return; }
  ...
  const handleForgotPassword = async () => {
    if (!resetEmail.trim()) { Alert.alert("Erreur", "Veuillez saisir votre adresse email."); return; }
    setResetLoading(true);
    try {
      const auth = getAuth();
      await sendPasswordResetEmail(auth, resetEmail.trim());
      Alert.alert("Email envoyé", "Un lien de réinitialisation a été envoyé à " + resetEmail.trim(),
        [{ text: "OK", onPress: () => { setShowForgotPassword(false); setResetEmail(""); } }]);
    } catch (error) {
      if (error.code === "auth/user-not-found") Alert.alert("Erreur", "Aucun compte associé à cet email.");
      else if (error.code === "auth/invalid-email") Alert.alert("Erreur", "Adresse email invalide.");
      else Alert.alert("Erreur", "Une erreur est survenue. Réessayez.");
    } finally {
  ...
  const handleForgotEmail = async () => {
    if (!searchPrenom.trim() || !searchTel.trim()) { Alert.alert("Erreur", "Veuillez remplir tous les champs."); return; }
    setSearchLoading(true);
    try {
      const db = getFirestore();
      const q = query(collection(db, "coaches"), where("firstName", "==", searchPrenom.trim()), where("telephone", "==", searchTel.trim()));
      const snapshot = await getDocs(q);
      if (snapshot.empty) {
        Alert.alert("Introuvable", "Aucun compte trouvé avec ces informations.");
      } else {
        const coach = snapshot.docs[0].data();
        const foundEmail = coach.email || "";
        const maskedEmail = maskEmail(foundEmail);
        Alert.alert("Compte trouvé", `Votre identifiant est : ${maskedEmail}\n\nConnectez-vous avec cet email.`,
          [{ text: "OK", onPress: () => { setShowForgotEmail(false); setSearchPrenom(""); setSearchTel(""); } }]);
      }
    } catch (error) {
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.form}>
            ...
            {/* Mot de passe oublié */}
            <TouchableOpacity style={styles.forgotPasswordLink} onPress={() => setShowForgotPassword(true)}>
            ...
            <View style={styles.footer}>
              <TouchableOpacity onPress={() => navigation.navigate("Register")}>
                <Text style={styles.footerText}>Pas encore de compte, <Text style={styles.footerLink}>créer ici</Text></Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.forgotEmailLinkWrapper} onPress={() => setShowForgotEmail(true)}>
                <Text style={styles.footerText}><Text style={styles.footerLink}>Identifiant oublié ?</Text></Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      <Modal visible={showForgotPassword} transparent animationType="fade" onRequestClose={() => setShowForgotPassword(false)}>
        <KeyboardAvoidingView style={styles.centeredModalOverlay} behavior={Platform.OS === "ios" ? "padding" : "height"}>
          <View style={styles.centeredModalContainer}>
            <Text style={styles.modalTitle}>Mot de passe oublié</Text>
            <Text style={styles.modalSubtitle}>Saisissez votre email, on vous envoie un lien de réinitialisation.</Text>
            <TextInput style={styles.input} placeholder="Votre adresse email" placeholderTextColor="#999" value={resetEmail} onChangeText={setResetEmail} keyboardType="email-address" autoCapitalize="none" editable={!resetLoading} />
            <TouchableOpacity style={[styles.loginButton, resetLoading && styles.loginButtonDisabled]} onPress={handleForgotPassword} disabled={resetLoading}>
              {resetLoading ? <ActivityIndicator color="#fff" /> : <Text style={styles.loginButtonText}>Envoyer le lien</Text>}
            </TouchableOpacity>
            <TouchableOpacity style={styles.modalCancelButton} onPress={() => { setShowForgotPassword(false); setResetEmail(""); }}>
      ...
      <Modal visible={showForgotEmail} transparent animationType="fade" onRequestClose={() => setShowForgotEmail(false)}>
        <KeyboardAvoidingView style={styles.centeredModalOverlay} behavior={Platform.OS === "ios" ? "padding" : "height"}>
          <View style={styles.centeredModalContainer}>
            <Text style={styles.modalTitle}>Identifiant oublié</Text>
            <Text style={styles.modalSubtitle}>Renseignez vos informations pour retrouver votre compte.</Text>
            <TextInput style={styles.input} placeholder="Prénom" placeholderTextColor="#999" value={searchPrenom} onChangeText={setSearchPrenom} autoCapitalize="words" editable={!searchLoading} />
            <TextInput style={styles.input} placeholder="Numéro de téléphone" placeholderTextColor="#999" value={searchTel} onChangeText={setSearchTel} keyboardType="phone-pad" editable={!searchLoading} />
            <TouchableOpacity style={[styles.loginButton, searchLoading && styles.loginButtonDisabled]} onPress={handleForgotEmail} disabled={searchLoading}>
              {searchLoading ? <ActivityIndicator color="#fff" /> : <Text style={styles.loginButtonText}>Retrouver mon compte</Text>}
            </TouchableOpacity>
            <TouchableOpacity style={styles.modalCancelButton} onPress={() => { setShowForgotEmail(false); setSearchPrenom(""); setSearchTel(""); }}>
              <Text style={styles.modalCancelText}>Annuler</Text>
            </TouchableOpacity>
          </View>
        </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  ...
  passwordWrapper: { flexDirection: "row", alignItems: "center", backgroundColor: "#fff", borderRadius: 8, borderWidth: 1, borderColor: "#ddd", marginBottom: 8, paddingRight: 12 },
